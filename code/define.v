`define RstEnable       1'b0
`define RstDisable      1'b1
`define RomEnable       1'b0 
`define RomDisable      1'b1
`define RamEnable       1'b0
`define RamDisable      1'b1
`define IOEnable        1'b0
`define IODisable       1'b1
`define RamWrite        1'b1
`define RamUnWrite      1'b0
`define IOWrite         1'b1
`define IOUnWrite       1'b0
`define Zero            0
`define Valid           1'b1
`define Invalid         1'b0
`define IntAssert       6'b000001
`define IntNotAssert    6'b000000

`define nop        6'b000000
`define logic      6'b001101

`define Inst_ori   6'b001101
`define Inst_addi  6'b001000
`define Inst_andi  6'b001100
`define Inst_xori  6'b001110
`define Inst_lui   6'b001111
`define Inst_reg   6'b000000
`define Inst_add   6'b100000
`define Inst_sub   6'b100010
`define Inst_and   6'b100100
`define Inst_or    6'b100101
`define Inst_xor   6'b100110
`define Inst_sll   6'b000000
`define Inst_srl   6'b000010
`define Inst_sra   6'b000011
`define Inst_j     6'b000010
`define Inst_jal   6'b000011
`define Inst_jr    6'b001000
`define Inst_jalr  6'b001001
`define Inst_beq   6'b000100
`define Inst_bne   6'b000101
`define Inst_bgtz  6'b000111
`define Inst_lw    6'b100011
`define Inst_sw    6'b101011
`define Inst_slt   6'b101010
`define Inst_bgtz  6'b000111
`define Inst_bltz  6'b000001
`define Inst_jalr  6'b001001
`define Inst_mult  6'b011000
`define Inst_multu 6'b011001
`define Inst_div   6'b011010
`define Inst_divu  6'b011011
`define Inst_mfhi  6'b010000
`define Inst_mflo  6'b010010
`define Inst_mthi  6'b010001
`define Inst_mtlo  6'b010011
`define Inst_ll    6'b110000
`define Inst_sc    6'b111000
`define Inst_Cp0   6'b010000
`define Inst_eret  6'b011000
`define Inst_syscall 6'b001100
`define Inst_mfc0  5'b00000
`define Inst_mtc0  5'b00100

`define Nop     6'b000000
`define Or      6'b000001
`define And     6'b000010
`define Xor     6'b000011
`define Add     6'b000100
`define Sub     6'b000101
`define Sll     6'b000110
`define Srl     6'b000111
`define Sra     6'b001000
`define J       6'b001001
`define Jal     6'b001010
`define Jr      6'b001011
`define Beq     6'b001100
`define Bne     6'b001100
`define Bgtz    6'b001101
`define Bltz    6'b001101
`define Lw      6'b001110
`define Sw      6'b001111
`define Lui     6'b010000
`define Mult    6'b010001
`define Multu   6'b010010
`define Div     6'b010011
`define Divu    6'b010100
`define Mfhi    6'b010101
`define Mflo    6'b010110
`define Mthi    6'b010111
`define Mtlo    6'b011000
`define Jalr    6'b011001
`define Slt     6'b011010
`define Ll      6'b011011
`define Sc      6'b011100
`define Mfc0    6'b011101
`define Mtc0    6'b011110
`define Eret    6'b011111
`define Syscall 6'b100000

`define Count   5'b01001
`define Compare 5'b01011
`define Status  5'b01100
`define Cause   5'b01101
`define EPC     5'b01110

`define CountInt 32'h0000_0001
`define SysInt   32'h0000_0010
`define ERET     32'h0000_0100

`define CountIPC 32'h0000_0E04
`define SysIPC      32'h0000_0E18

`define KEY        32'h0000_1000
`define BUTTON     32'h0000_1004
`define SEG        32'h0000_1008
`define LED        32'h0000_100C